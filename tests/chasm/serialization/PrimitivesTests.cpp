...
BOOST_AUTO_TEST_SUITE(serialization_of_primitives_and_compounds)
    ...
    template <typename T>
    void compare_vectors(std::vector<T> const& input, std::vector<std::byte> const& result, std::function<std::byte(T const&)> converter = [](T const& obj){return obj;}){
    ...
    BOOST_FIXTURE_TEST_CASE(serializes_integers, EnvFixture) {
        ...
        for(auto i = 0; i < sizeof(anUnion.bytes); ++i) BOOST_CHECK(anUnion.bytes[i] == static_cast<uint8_t>(result.at(i)));
    ...
    BOOST_FIXTURE_TEST_CASE(serializes_a_vector_of_unique_ptrs_of_bytes, EnvFixture){
        ...
        std::function<std::byte(std::unique_ptr<std::byte> const&)> converter = [](std::unique_ptr<std::byte> const& obj)-> std::byte{return *obj;};
